...
def test_br_query_never_returns_global_chunks() -> None:
    ...
    results = asyncio.run(
        retriever.retrieve("Getnet products and payments", market=Market.BR, top_k=10)
    )
...
def test_global_query_never_returns_br_chunks() -> None:
    retriever = LocalKnowledgeRetriever()
    results = asyncio.run(
        retriever.retrieve("Getnet produtos e taxas", market=Market.GLOBAL, top_k=10)
    )
...
def test_unrelated_query_scores_zero() -> None:
    retriever = LocalKnowledgeRetriever()
    results = asyncio.run(
        retriever.retrieve("xyzzy unrelated gibberish nonsense", market=Market.BR, top_k=3)
    )
...
def test_ofertas_chunk_is_marked_high_volatility() -> None:
    retriever = LocalKnowledgeRetriever()
    results = asyncio.run(
        retriever.retrieve("ofertas taxas mensalidade", market=Market.BR, top_k=10)
    )
